...
function xpressui_render_workflows_page() {
	...
	if ( ! empty( $visible_installed_slugs ) ) {
		...
		// Pass settings map to the enqueued admin-workflows.js.
		wp_add_inline_script(
			'xpressui-bridge-admin-wf',
			'xpressuiBridgeAdmin.settingsMap = ' . $settings_json . ';', // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- wp_json_encode output
			'before'
		);
		echo '</div>';
	}
