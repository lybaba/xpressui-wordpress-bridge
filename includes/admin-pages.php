...
function xpressui_render_workflows_page() {
	...
	if ( isset( $_POST['xpressui_upload_pack'] ) && check_admin_referer( 'xpressui_upload_action', 'xpressui_nonce' ) ) {
		if ( xpressui_get_current_runtime_tier() !== 'pro' ) {
			$result = new WP_Error( 'pro_required', __( 'Installing custom workflow packs requires the XPressUI Pro extension with a valid license.', 'xpressui-bridge' ) );
		} else {
	...
	if ( xpressui_get_current_runtime_tier() !== 'pro' ) {
		echo '<p>' . esc_html__( 'Installing custom workflow packs is a Pro feature.', 'xpressui-bridge' ) . '</p>';
		echo '<p><strong>' . esc_html__( 'Upgrade to XPressUI Pro', 'xpressui-bridge' ) . '</strong> ' . esc_html__( 'to upload and manage your own workflow packs generated by the XPressUI console.', 'xpressui-bridge' ) . '</p>';
		echo '<p><a href="https://iakpress.com/xpressui-pro" target="_blank" rel="noreferrer" class="button button-primary">' . esc_html__( 'Get XPressUI Pro →', 'xpressui-bridge' ) . '</a></p>';
	} else {
