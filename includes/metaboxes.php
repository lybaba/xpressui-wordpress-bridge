...
function xpressui_render_status_metabox( $post ) {
	...
	// Status is editable in WordPress: submissions are managed locally (the free/standalone
	// inbox), and operators move them New → In review → Done here. On a Console-connected site
	// a later sync from the Console can still update it.
	$status_options = xpressui_get_status_options();
	echo '<label for="xpressui_submission_status"><strong>' . esc_html__( 'Status', 'xpressui-bridge' ) . '</strong></label><br />';
	echo '<select id="xpressui_submission_status" name="xpressui_submission_status" class="xpressui-full-width">';
	foreach ( $status_options as $value => $label ) {
		echo '<option value="' . esc_attr( (string) $value ) . '"' . selected( $current_status, $value, false ) . '>' . esc_html( (string) $label ) . '</option>';
	}
	echo '</select>';
	echo '<p class="xpressui-hint">' . esc_html__( 'Set the workflow status, then click Update.', 'xpressui-bridge' ) . '</p>';
...
function xpressui_save_submission_status( $post_id ) {
	...
	// Status is editable from the Submission Workflow metabox. Fall back to the current
	// status (then 'new') when nothing valid is posted; validated against the options below.
	$status      = isset( $_POST['xpressui_submission_status'] )
		? sanitize_key( wp_unslash( (string) $_POST['xpressui_submission_status'] ) )
		: ( $previous_status !== '' ? $previous_status : 'new' );
...
function xpressui_render_ref_file_picker_row( $field_name, $ref_files ) {
	...
	echo '</button>';
		if ( $has_file ) {
			echo '<span class="xpressui-ref-file-preview" data-field="' . esc_attr( $field_name ) . '">';
		} else {
			echo '<span class="xpressui-ref-file-preview" hidden data-field="' . esc_attr( $field_name ) . '">';
		}
	if ( $file_url !== '' ) {
...
function xpressui_render_delivery_metabox( $post ) {
	...
	$badge = static function( string $status, bool $is_saas_handling = false ): string {
		...
		$fmt_date = static function( string $iso ): string {
			...
			$ts = strtotime( $iso );
			return $ts ? (string) wp_date( 'M j, Y · H:i', $ts ) : $iso;
		};
	...
	if ( $webhook_status === '' || $webhook_status === 'not-set' ) {
		...
		if ( $webhook_code !== '' ) {
			...
			echo '<div class="xpressui-dl-row"><span class="xpressui-dl-key">' . esc_html__( 'Sent at', 'xpressui-bridge' ) . '</span><span>' . esc_html( $fmt_date( $webhook_sent_at ) ) . '</span></div>';
		if ( $webhook_error !== '' ) {
	...
	if ( empty( $event_log ) ) {
		...
		foreach ( array_slice( array_reverse( $event_log ), 0, 8 ) as $event ) {
			...
			echo '<span class="xpressui-event-meta">' . esc_html( $fmt_date( $occurred_at ) ) . ' · ' . esc_html( $source ) . '</span>';
			echo '</li>';
		}
		echo '</ul>';
	}
